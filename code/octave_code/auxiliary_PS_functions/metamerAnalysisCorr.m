function params = metamerAnalysisCorr(oim,m,opts)
  ...
  for nor = 1:Nor,
    ...
    for imask=1:m.scale{nsc}.nMasks
      ...
      if normalizeCovars
        vari = ac(ceil(size(ac,1)/2),ceil(size(ac,1)/2));
        ac = ac / vari;
        variFull = acFull(ceil(size(acFull,1)/2),ceil(size(acFull,1)/2));
        acFull = acFull / variFull;
      end
      %
...
for nsc = 1:Nsc,
  ...
  for imask = 1:m.scale{nsc}.nMasks;
    ...
    for inc=1:nc
      ...
      cousins_SD(inc) = sqrt(wvar2(cousins(:,inc),vector(thisMask)));
    end
    for inp=1:np
      parentstmp(:,inp) = (parents(:,inp) - wmean2(parents(:,inp),vector(thisMask))).*sqrt(vector(thisMask)/sum(thisMask(:)));
      parents_SD(inp) = sqrt(wvar2(parents(:,inp),vector(thisMask)));
    end

    % DH normalize C0
    if normalizeCovars
      C0tmp = innerProd(cousinstmp)./(cousins_SD' * cousins_SD);
      % keep diagonal elements
      C0tmp(logical(eye(size(C0tmp)))) = (cousins_SD.^2);
      C0.scale{nsc}.mask{imask}(1:nc,1:nc) = C0tmp;
    else
      % DH took out division by cousinSz, doesn't seem to belong
      C0.scale{nsc}.mask{imask}(1:nc,1:nc) = innerProd(cousinstmp);
      % Freeman original line
      %C0.scale{nsc}.mask{imask}(1:nc,1:nc) = innerProd(cousinstmp)/cousinSz;
    end

    if (np > 0)
      if normalizeCovars
        Cx0tmp =(cousinstmp' * parentstmp)./(cousins_SD' * parents_SD);
        Cx0.scale{nsc}.mask{imask}(1:nc,1:np) = Cx0tmp;
      else
        % Freeman original line
        Cx0.scale{nsc}.mask{imask}(1:nc,1:np) = (cousinstmp'*parentstmp)/cousinSz;
        % DH took out division by cousinSz, doesn't seem to belong
        Cx0.scale{nsc}.mask{imask}(1:nc,1:np) = (cousinstmp'*parentstmp);
      end
      if (nsc==Nsc)
        % Original Freeman
        %C0.scale{Nsc+1}.mask{imask}(1:np,1:np) = innerProd(parentstmp)/(cousinSz/4); % divide by 1/4?;
        % DH modified to take out cousinsSz
        C0.scale{Nsc+1}.mask{imask}(1:np,1:np) = innerProd(parentstmp)/(1/4); % divide by 1/4?;
      end
    end
  ...
  for imask = 1:m.scale{nsc}.nMasks;
    ...
    for inrp=1:nrp
      ...
      % added DH
      rparents_SD(inrp) = sqrt(wvar2(rparents(:,inrp),vector(thisMask)));
    end

    if normalizeCovars
      Cr0temp= innerProd(cousinstmp)./(cousins_SD' * cousins_SD);
      Cr0temp(logical(eye(size(Cr0temp)))) = (cousins_SD.^2);
      Cr0.scale{nsc}.mask{imask}(1:nrc,1:nrc) = Cr0temp;
    else
      % Modified DH
      Cr0.scale{nsc}.mask{imask}(1:nrc,1:nrc) = innerProd(cousinstmp);
      % Original line Freeman
      % Cr0.scale{nsc}.mask{imask}(1:nrc,1:nrc) = innerProd(cousinstmp)/cousinSz;
    end
    if (nrp > 0)
      if normalizeCovars
        Crx0temp = (cousinstmp'*rparentstmp)./(cousins_SD' * rparents_SD);
        Crx0.scale{nsc}.mask{imask}(1:nrc,1:nrp) = (Crx0temp);
      else
        % Modified DH
        Crx0.scale{nsc}.mask{imask}(1:nrc,1:nrp) = (cousinstmp'*rparentstmp);
        % Original line Freeman
        %Crx0.scale{nsc}.mask{imask}(1:nrc,1:nrp) = (cousinstmp'*rparentstmp)/cousinSz;
      end
      Crx0.scale{nsc}.mask{imask}(1:nrc,1:nrp) = (cousinstmp'*rparentstmp)/cousinSz;
      if (nsc==Nsc)
        Cr0.scale{Nsc+1}.mask{imask}(1:nrp,1:nrp) = innerProd(rparentstmp)/(1/4); % divide by 1/4?
        % Original Freeman line
...
for imask=1:m.scale{1}.nMasks
  ...
  % DH do correlation
  if normalizeCovars
    centerCoord = ceil(size(ac,1)/2);
    ac = ac / ac(centerCoord, centerCoord);
  end
  %
